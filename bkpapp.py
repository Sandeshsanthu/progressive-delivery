...
def init_db():
    ...
    db.execute("CREATE INDEX IF NOT EXISTS idx_purchase_items_purchase ON purchase_items(purchase_id);")
    db.execute("CREATE INDEX IF NOT EXISTS idx_purchase_items_listing ON purchase_items(listing_id);")
...
def load_cart_items(conn):
    ...
    q = ",".join(["?"] * len(ids))
    rows = conn.execute(
        f"SELECT id, title, price_cents, status FROM listings WHERE id IN ({q})",
        ids,
    ).fetchall()
...
def add_to_cart(listing_id: int):
    ...
    db = get_db()
    row = db.execute("SELECT id, status FROM listings WHERE id = ?", (listing_id,)).fetchone()
    if not row:
        abort(404)
...
def pay():
    ...
    try:
        ...
        available = db.execute(
            f"SELECT COUNT(*) AS c FROM listings WHERE id IN ({q}) AND status = 'ACTIVE'",
            ids,
        ).fetchone()["c"]
        ...
        for i in items:
            db.execute(
                "INSERT INTO purchase_items (purchase_id, listing_id, price_cents) VALUES (?, ?, ?)",
                (purchase_id, int(i["id"]), int(i["price_cents"])),
            )

        db.execute(
            f"UPDATE listings SET status = 'SOLD', updated_at = ? WHERE id IN ({q})",
            (now, *ids),
        )
